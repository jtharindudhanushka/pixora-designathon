...
export type EnergyMode = 'IDLE' | 'PRE_COOLING' | 'ECO_FLOAT';

export interface EnergyRuleState {
  id: string;
  label: string;
  description: string;
  enabled: boolean;
...
export interface EnergyStatus {
  ...
  nowPct: number;
  capKw: number;
  cycleSavingUsd: number;
  monthSavingsLkr: number;
  savingsVsStandardPct: number;
  bannerVisible: boolean;
  bannerAutoClearsLabel: string;
  livingAcTemp: number;
  livingAcState: string;
  rules: EnergyRuleState[];
  strategyText: string;
...
function fmtLabel(totalMinutes: number): string {
...
function pctWithin(totalMinutes: number): number {
...
export class EnergyOptimizer {
  private rules: Record<string, EnergyRuleState> = {
    thermalPreCooling: {
  ...
  private debugModeOverride: EnergyMode | null = null;

  setDebugMode(mode: EnergyMode | null): void {
    this.debugModeOverride = mode;
  }

  setRule(ruleId: string, enabled: boolean): EnergyRuleState | null {
  ...
  getRules(): EnergyRuleState[] {
    return Object.values(this.rules);
  }

  getStatus(livingAc: { state: string; temp: number }): EnergyStatus {
    const now = new Date();
