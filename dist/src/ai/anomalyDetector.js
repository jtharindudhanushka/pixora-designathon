...
export class AnomalyDetector {
    ...
    manualToleranceMultiplier = 1.0; // Widened when resident taps [Dismiss / Heavy Usage]
    manualOverrideExpiresAt = 0;
    constructor() {
    ...
    processTelemetry(packet) {
        ...
        const variance = this.history.reduce((sum, h) => sum + Math.pow(h.dropRate - mean, 2), 0) / n;
        ...
        if (isAnomaly) {
            ...
            explainabilityText = `Smart Lock #1402 voltage is dropping ${multiplier}x faster than normal actuation baseline (${currentDropRate.toFixed(1)} mV/actuation vs ${mean.toFixed(1)} mV/actuation baseline). Internal cell failure predicted in ~${predictedHoursToFailure} hours. Preventive replacement ticket auto-routed to Facilities.`;
        }
            ...
            explainabilityText = `Device health optimal. Voltage degradation is within 1.1σ of 14-day rolling baseline (${mean.toFixed(1)} mV/actuation).`;
        }
        const report = {
            isAnomaly,
     ...
     */
    applyManualOverride(durationHours = 24, toleranceMultiplier = 1.6) {
