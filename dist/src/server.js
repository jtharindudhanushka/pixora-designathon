...
async function bootstrap() {
    ...
    const executeAction = (target, action, payload) => {
        if (target === 'device' && action === 'UPDATE_DEVICE') {
            ...
            if (deviceId === 'living-ac') {
                ...
                if (temp)
                    hardwareDaemon.unitDevices.guestRoomAc.temp = temp;
    ...
    const publishToMqtt = (topic, msg) => mqttBroker.publish(topic, msg);
    const broadcastToWs = (type, data) => wsHub.broadcast(type, data);
