...
export function createDeviceRouter(hardwareDaemon, broadcastWs) {
    const router = Router();
    /**
     * GET /api/devices
    ...
    router.post('/:id/control', (req, res) => {
        ...
        if (id === 'living-ac') {
            ...
            if (temp !== undefined)
                hardwareDaemon.unitDevices.livingRoomAc.temp = Math.max(18, Math.min(28, temp));
        ...
        if (id === 'guest-ac') {
            ...
            if (temp !== undefined)
                hardwareDaemon.unitDevices.guestRoomAc.temp = Math.max(18, Math.min(28, temp));
